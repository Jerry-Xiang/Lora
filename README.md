...
`training_log_YYYYMMDD_HHMMSS.txt` - 为训练过程统计信息日志。包含训练时间、样本数、步骤数、训练损失等信息，以及每个 epoch 的训练损失，logs中包含两个日志文件。
-日期早的日志文件执行微调后识别图片还是存在缺陷，部分信息识别不对，需要检查。日期最新的日志文件为最终成功训练的日志，识别了图片所有指定信息，包括里程表信息。
